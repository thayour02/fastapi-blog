...
from schema import UserCreate,UserPublic,UserPrivate,Token, UserUpdate,PostResponse,PaginatedPostsResponse,ChangePasswordRequest,ForgotPasswordRequest,ResetPasswordRequest
...
from auth import CurrentUser,verify_password,generate_reset_token,hash_reset_token,create_access_token,hash_password
...
async def login_for_access_token(
    ...
    result = await db.execute(select(model.User).where(
...
## get_user_posts
@router.get("/{user_id}/post", response_model=PaginatedPostsResponse)
...
async def update_user(
        ...
        if user_update.username is not None and user_update.username.lower() != current_user.username.lower():
            result = await db.execute(
                select(model.User).where(
                    func.lower(model.User.username) == user_update.username.lower()
                ),
            )
            existing_user = result.scalars().first()
            if existing_user:
        ...
        if user_update.email is not None and user_update.email.lower() != current_user.email.lower():
            result = await db.execute(
                select(model.User).where(
                    func.lower(model.User.email) == user_update.email.lower()
                ),
            )
            existing_email = result.scalars().first()
            if existing_email:
