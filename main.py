...
posts: list[dict] = [
...

    {
        ...
        "content":"fastapi framework make it even better",
...
def create_user(user:UserCreate, db:Annotated[Session, Depends(get_db)]):
    ...
    result = db.execute(select(model.User.username == user.username))
    user_exists = result.scalars().first()
    if user_exists:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User already exists")
    ## checking existing email
    result = db.execute(select(model.User.email == user.email))
...
def api_get_user(user_id: int, db:Annotated[Session, Depends(get_db)]):
    result = db.execute(select(model.User).where(model.User.id == user.id))
...
def api_get_post(post_id: int, db:Annotated[Session, Depends(get_db)]):
    result = db.execute(select(model.Post).where(model.Post.id == post_id))
    post = result.scalars().first()
    if post:
        return post
